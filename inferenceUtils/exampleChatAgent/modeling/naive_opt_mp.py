...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        # final_layer_norm to last device
        if self.final_layer_norm:
            self.final_layer_norm = self.final_layer_norm.to(self.last_device)

        # project_in to first device
        if self.project_in:
            self.project_in = self.project_in.to(self.first_device)

        # project_out to last device, same as final_layer_norm
        if self.project_out:
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # TODO: AttributeError: 'tuple' object has no attribute 'to'
            # if self.model_parallel and past_key_value:
            #     past_key_value = past_key_value.to(hidden_states.device)
...
class NaiveMP_OPTForCausalLM(OPTPreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        assert_device_map(self.device_map, len(self.model.decoder.layers))
...
if __name__ == "__main__":
    ...
    model = NaiveMP_OPTForCausalLM.from_pretrained("facebook/opt-125m")
    model.parallelize()
    print(len(model.model.decoder.layers))
    print(model.model.decoder.padding_idx)

    tokenizer = GPT2Tokenizer.from_pretrained("facebook/opt-125m")

    inputs = tokenizer("Hello, my dog is cute", return_tensors="pt", padding="max_length", max_length=7)
    ...
    print(outputs.loss)  # expected 4.8475

    # outputs = model(**inputs)

    # gpt_tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
    # print (gpt_tokenizer("Hello, my dog is cute"))
